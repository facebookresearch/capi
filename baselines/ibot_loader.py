...
# adapted from https://github.com/bytedance/ibot/blob/main/models/vision_transformer.py
import logging
import math
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.norm = nn.Identity() if use_mean_pooling else partial(nn.LayerNorm, eps=1e-6)(embed_dim)
        self.fc_norm = partial(nn.LayerNorm, eps=1e-6)(embed_dim) if use_mean_pooling else None
        # Classifier head
    ...
    def forward(  # type: ignore
        ...
        for blk in self.blocks:
            x = blk(x)
        x = self.norm(x)
        patches = x[:, 1:].reshape(B, h // self.patch_size, w // self.patch_size, -1)
        return x[:, 0], None, patches


def vit_tiny(patch_size=16, **kwargs):
    model = VisionTransformer(
        patch_size=patch_size, embed_dim=192, depth=12, num_heads=3, mlp_ratio=4, qkv_bias=True, **kwargs
    )
    return model
...
model_dict = {
    ...
    "ibot_vitl16": (
        vit_large,
        "https://lf3-nlp-opensource.bytetos.com/obj/nlp-opensource/archive/2022/ibot/vitl_16_rand_mask/checkpoint_teacher.pth",
        "state_dict",
        "",
    ),
    "ibot_vitb16_in22k": (
        vit_base,
        "https://lf3-nlp-opensource.bytetos.com/obj/nlp-opensource/archive/2022/ibot/vitb_16_pt22k/checkpoint_student.pth",
        "state_dict",
        "",
    ),
    "ibot_vitl16_in22k": (
        vit_large,
        "https://lf3-nlp-opensource.bytetos.com/obj/nlp-opensource/archive/2022/ibot/vitl_16_pt22k/checkpoint_student.pth",
        "",
    ),
}
...
def __model_loader__(model_name: str, device: str = "cuda") -> nn.Module:
    method, url, *keys, prefix = model_dict[model_name]
