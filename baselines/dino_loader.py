...
class DINOv1Adapter(nn.Module):
    ...
    def forward(  # type: ignore
        ...
        B, _, h, w = x.shape
        x = self._model.prepare_tokens(x)
...
def __model_loader__(model_name: dino_name_type, device: str = "cuda") -> nn.Module:
    model: nn.Module = globals()[model_name]()
    model = DINOv1Adapter(model)
    model.eval()
